...
\paragraph{}
The above definition can be similarly defined for extensions of level $l$: a crossing of the $(f,E_r)$-extension of level $l$, $d_{n}^{f,E_r,l}(x)=y$, is defined as an essential $(f, E_{r-a})$-extension of level $l$ from some $x'\in Z_{r-1-a}^{s+a,t+a}(X)$ to
$$y'\in Z_{r-1-n+b+e(f)}^{s+n-b,t+n-b}(Y)\backslash B_{1+n-b-e(f)}^{s+n-b,t+n-b}(Y)$$
...
\begin{proof}
    The proof is analogous to that of Proposition~\ref{prop:cross-f-Er}. A crossing of the synthetic $\hat f_{r-1}$-extension $d_n^{\hat f_{r-1}}(\lambda^l x)=\lambda^{n-e(f)+l}y$ has the form (\ref{eq:cross-level}). Consider the commutative diagram
    $$\xymatrix{
        \Sus^{0,e(f)}\nu X/\lambda^{r-1-a} \ar[r]^-{\hat f_{r-1-a}} \ar[d]_{\lambda^a} & \nu Y/\lambda^{r-1-a} \ar[d]^{\lambda^a}\\
        \Sus^{0,e(f)+a}\nu X/\lambda^{r-1}\ar[r]_-{\hat f_{r-1}} & \Sus^{0,a}\nu Y/\lambda^{r-1}.\\
    }$$
    Lifting (\ref{eq:cross-level}) along $\lambda^a$ yields a differential
    $$d_{n-a-b}^{\hat f_{r-1-a}}(\lambda^l x')=\lambda^{n-a-b-e(f)+l}y''$$
    for some $y''$ with $y''\equiv y'\mod B_{1+n-b-e(f)}^{s+n-b,t+n-b}(Y)$, which is essential if and only if (\ref{eq:cross-level}) is essential. This is precisely a crossing of the $(f,E_r)$-extension of level $l$ at $x$.
\end{proof}
